...
import csv
from tensorflow import keras        feature_layer = Conv2D(n_filters, (1, 1), strides=(1, 1), padding='same', data_format=IMAGE_ORDERING)(feature_layer)
        feature_layer = ReLU()(feature_layer)

        feature_layer = SeparableConv2D(n_filters, (3, 3), strides=(2, 2), padding='same', data_format=IMAGE_ORDERING)(feature_layer)
        feature_layer = ReLU()(feature_layer)

        feature_layer = Conv2D(n_filters, (1, 1), strides=(1, 1), activation='linear', padding='same', data_format=IMAGE_ORDERING)(feature_layer)
...
max_layers = 0
for layers in range(18,7,-1):
# each case tries six different networks
